...
import { ArrowLeft, Zap, Share2, HardDrive, FileIcon, RefreshCw, Trash2, CheckCircle } from "lucide-react";
...
export default function AdminPage() {
  ...
  const handleCleanup = async () => {
    ...
    try { const { message } = await triggerCleanup(); setCleanMsg(message); await fetchStats(); }
    catch (e: unknown) { setCleanMsg(e instanceof Error ? e.message : "Cleanup failed"); }
    ...
    <div className="min-h-screen flex flex-col bg-light-bg dark:bg-dark-bg transition-colors duration-200">
          ...
          <div className="flex items-center gap-2">
            <button onClick={fetchStats} disabled={loading}
              className="p-2 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300
                hover:bg-light-elevated dark:hover:bg-dark-elevated transition-colors disabled:opacity-50"
              title="刷新">
      ...
      <main className="flex-1 px-4 py-8">
        <div className="max-w-3xl mx-auto space-y-6">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t.adminStats}</h1>
          ...
          {stats && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { icon: Share2, label: t.activeShares, value: stats.active_shares.toLocaleString(), color: "text-brand-blue" },
                { icon: FileIcon, label: t.totalFiles, value: stats.total_files.toLocaleString(), color: "text-purple-500" },
                { icon: HardDrive, label: t.storageUsed, value: stats.storage_used_human, color: "text-emerald-500" },
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleCleanup} disabled={cleaning}
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium
                ...
                text-red-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
              <Trash2 size={15} />
          ...
          {stats && stats.recent_shares.length > 0 && (
            <div className="rounded-xl overflow-hidden
              ...
              <div className="px-5 py-3 border-b border-light-border dark:border-dark-border">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{t.recentShares}</h3>
              </div>
              <div className="divide-y divide-light-border dark:divide-dark-border">
                {stats.recent_shares.map((s) => (
                  <div key={s.code} className="flex items-center gap-4 px-5 py-3">
                    <span className="font-mono text-sm font-bold text-brand-blue w-20 flex-shrink-0">{s.code}</span>
                    <span className="text-xs text-gray-400 w-12 flex-shrink-0">{s.type}</span>
                    <span className="text-xs text-gray-500 flex-1">{formatDate(s.created_at)}</span>
                    <span className="text-xs text-gray-400">{s.download_count} 次取件</span>
                    <span className="text-xs text-gray-400 hidden sm:block">到期 {formatDate(s.expire_at)}</span>
                  </div>
                ))}
              </div>
            </div>
