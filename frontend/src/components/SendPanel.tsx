...
export default function SendPanel() {
  ...
  const handleSubmit = async () => {
    if (!canSubmit) return;
    setError(""); setUploading(true); setProgresses([]);
    try {
      let uploadIds: string[] = [];
      if (files.length > 0) {
        setProgresses(files.map((f) => ({
          filename: f.name, totalSize: f.size, uploadedBytes: 0,
          percentage: 0, speed: 0, remainingSeconds: 0, status: "pending" as const,
        })));
        uploadIds = await uploadFiles(files, (idx, progress) => {
          setProgresses((prev) => { const u = [...prev]; u[idx] = progress; return u; });
        });
      }
  ...
  const handleReset = () => {
    setResult(null); setFiles([]); setText(""); setPassword("");
    setCustomCode(""); setMaxDownloads(0); setExpireSeconds(86400);
    setProgresses([]); setError("");
  };
  ...
  const btnBase = "px-4 py-1.5 rounded-lg text-sm transition-all duration-200";
  const btnActive = "bg-brand-blue text-white";
  const btnInactive = `bg-light-elevated dark:bg-dark-elevated border border-light-border dark:border-dark-border
    text-gray-500 dark:text-gray-400 hover:border-brand-blue/60 hover:text-gray-900 dark:hover:text-white`;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Text area */}
      <div>
        <p className="text-xs text-gray-400 mb-2">{t.orPasteText}</p>
        <textarea value={text} onChange={(e) => setText(e.target.value)}
          disabled={uploading} placeholder={t.textPlaceholder} rows={4}
      ...
      {/* Expire */}
      <div>
        <label className="text-xs text-gray-400 mb-2 block">{t.expireLabel}</label>
        <div className="flex flex-wrap gap-2">
          {EXPIRE_OPTIONS.map((opt) => (
            <button key={opt.value} onClick={() => setExpireSeconds(opt.value)} disabled={uploading}
              className={`${btnBase} ${expireSeconds === opt.value ? btnActive : btnInactive}`}>
      ...
      {showAdvanced && (
        <div className="space-y-4 animate-slide-up pt-1">
          {/* Password */}
          <div>
            <label className="text-xs text-gray-400 mb-2 block">{t.passwordLabel}</label>
            <input type="text" value={password} onChange={(e) => setPassword(e.target.value)}
              disabled={uploading} placeholder={t.passwordPlaceholder}
          ...
          {/* Max downloads */}
          <div>
            <label className="text-xs text-gray-400 mb-2 block">{t.maxDownloadsLabel}</label>
            <div className="flex flex-wrap gap-2">
              {MAX_DOWNLOAD_OPTIONS.map((n) => (
                <button key={n} onClick={() => setMaxDownloads(n)} disabled={uploading}
                  className={`${btnBase} px-3 ${maxDownloads === n ? btnActive : btnInactive}`}>
          ...
          {/* Custom code */}
          <div>
            <label className="text-xs text-gray-400 mb-2 block">{t.customCodeLabel}</label>
            <input type="text" value={customCode}
              onChange={(e) => setCustomCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))}
              disabled={uploading} placeholder={t.customCodePlaceholder} maxLength={20}
